...
DB_FILE = "/home/barnsley/ELT-PCS/mech/SWIFT.QrtMeasDb"
...
for l in lenses:
  ...
  for entry in db[t_coordsys]['data']:
    this_csMsRecNr = int(entry[db[t_coordsys]['headers'].index('csMsRecNr')])
    for csys in cfg['COORDINATE_SYSTEMS']:
      if csys['csMsRecNr'] == this_csMsRecNr:	# do we actually use this coordinate system?
	...
	# actual values stored in database are the transformed machine coordinates, so 
        # the transform matrix is the coordinate transform required to convert back to
        # a specific coordinate system, i.e. we need to take the INVERSE
